  ...
  try {
    ...
    const tokenResponse = await fetch('https://api.pinterest.com/v5/oauth/token', {
      ...
      body: new URLSearchParams({
        ...
        code,
        redirect_uri: `${Deno.env.get('SUPABASE_URL')}/functions/v1/pinterest-auth-callback`,
        client_id: Deno.env.get('PINTEREST_CLIENT_ID') || '',
        client_secret: Deno.env.get('PINTEREST_CLIENT_SECRET') || '',
      }),
    });
    ...
    const accountsResponse = await fetch('https://api.pinterest.com/v5/ad_accounts', {
      headers: {
